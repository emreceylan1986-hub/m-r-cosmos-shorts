#!/usr/bin/env python3
"""
trailer_uret.py — CosmoBytes kanal trailer (~45 sn) üretir.

Çıktı:
    branding/trailer.mp4    (1080x1920, ~45 sn, ses + altyazı + ken burns)
    branding/trailer.mp3    (ham TTS)
    branding/trailer.txt    (script)

Kullanım: python3 trailer_uret.py
Sonra: python3 trailer_yukle.py
"""
import asyncio
import shutil
import subprocess
...
# 45 saniyelik anchor — sabit (Gemini değil), kalite garantili
...
Three new shorts every day, at 3 PM, 7 PM, and 10 PM.

For the dreamers. For the wanderers. For the curious mind.

Hit subscribe. See you in the cosmos."""

# Altyazı için satır kırma (her satır ~3 sn ekranda)
SATIRLAR = [
    ("Welcome to", 1.8),
    ("CosmoBytes", 1.8),
    ("Your daily dose", 1.6),
    ("of cosmic wonder", 1.8),
    ("in 30 seconds", 1.6),
    ("Black holes", 1.5),
    ("that bend time", 1.7),
    ("Galaxies", 1.4),
    ("older than memory", 2.0),
    ("Exoplanets", 1.5),
    ("too strange", 1.5),
    ("to imagine", 1.6),
    ("Three new shorts", 1.7),
    ("every day", 1.4),
    ("at 3 PM, 7 PM", 2.0),
    ("and 10 PM", 1.7),
    ("For the dreamers", 1.7),
    ("For the wanderers", 1.7),
    ("For the curious mind", 2.0),
    ("Hit subscribe", 2.0),
    ("See you in the cosmos", 2.5),
]
...
def ass_dosyasi_yaz(ass_yol: Path, sure: float):
    """Smart altyazı — banner satırları toplam süreye dağıt."""
...
def banner_dikey(banner_2048: Path, cikti_dikey: Path):
    """Banner 2048x1152'yi 1080x1920 dikey arka plan'a uyarla — blur+overlay."""
...
def video_montaj(bg_dikey: Path, mp3: Path, ass: Path, cikti: Path, sure: float):
    """Ken Burns yakınlaşma + ses + altyazı = final mp4. WAV ara dosyası ile."""
