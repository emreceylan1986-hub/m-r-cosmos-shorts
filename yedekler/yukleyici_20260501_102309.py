"""
yukleyici.py — YouTube Shorts Otomatik Yükleyici

Akış (her adım terminale yazılır):
    1) En son MP4 + senaryo TXT bulunur (zaman damgası eşleşmeli)
    2) Gemini başlık + açıklama + 5 etiket üretir
    3) bridge.metin_onay_iste ile metin paketi denetlenir, REVIZE varsa kullanılır
    4) OAuth: token.json varsa refresh, yoksa client_secret.json ile
       browser-based ilk-açılış flow tetiklenir
    5) Video YouTube'a yüklenir — VARSAYILAN: privacyStatus="private"
       (Studio'da kontrol edilip yayınlanır; --gizlilik public|unlisted ile değişir)
    6) Video ID + Studio link terminale yazılır

Komut satırı:
    python yukleyici.py                      # default private
    python yukleyici.py --gizlilik unlisted
    python yukleyici.py --gizlilik public --made-for-kids no
"""
...
def en_son_video_ve_senaryo() -> tuple[Path, str]:
    ...
    if not txt.exists():
        raise FileNotFoundError(f"Eşleşen senaryo yok: {txt.name}")
...
def metadatayi_denetlet(veri: dict, senaryo: str) -> dict:
    ...
    yt = re.search(r"TITLE:\s*(.+)", yeni)
    yd = re.search(r"DESCRIPTION:\s*(.*?)\n\nTAGS:", yeni, re.DOTALL)
    yg = re.search(r"TAGS:\s*(.+)", yeni)
    if yt: veri["title"] = yt.group(1).strip()
    if yd: veri["description"] = yd.group(1).strip()
    if yg: veri["tags"] = [t.strip() for t in yg.group(1).split(",") if t.strip()]
    if "#Shorts" not in veri["description"]:
        veri["description"] = veri["description"].rstrip() + "\n\n#Shorts"
    return veri
