...
HIZ = "-10%"       # 19 Haz: 28-32sn videoları 45-55sn'e uzat
...
SPACE / ASTRONOMY / COSMIC-FACTS niche. Your job is RETENTION — the first
...
embellishment of well-established space/astronomy/cosmic facts.
...
- HOOK (first sentence, MAX 8 words): a punchy, curiosity-gap opener about
  ...
  ── REGISTERED VIRAL PATTERN (CosmoBytes kanal verisi 2026-Haz):
     ...
     "Pluto is smaller than the USA.", "Jupiter could fit 1,300 Earths."
...
def _karaoke_ass(cues: list[tuple[int, int, str]], grup: int = 3) -> str:
    ...
    bas = (
        ...
        # Font 64 → 56 (uzun kelimeler için ekstra alan)
    ...
    def _akilli_grupla(kelimeler):
        ...
        for k in kelimeler:
            if len(k) >= MAX_KARAKTER:
                if mevcut:
                    gruplar.append(mevcut); mevcut = []; mevcut_uzunluk = 0
                gruplar.append([k]); continue
            yeni = mevcut_uzunluk + len(k) + (1 if mevcut else 0)
            if yeni > MAX_KARAKTER and mevcut:
                gruplar.append(mevcut); mevcut = [k]; mevcut_uzunluk = len(k)
            else:
                mevcut.append(k); mevcut_uzunluk = yeni
