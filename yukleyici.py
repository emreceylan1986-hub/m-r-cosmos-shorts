...
Schema:
{
  ...
  "description": "200-400 characters TOTAL. Structure:
    ...
    - Last line: 4-6 hashtags. ALWAYS include #Shorts, then 3-5 niche tags
      like #space #astronomy #cosmos #science #universe #didyouknow #shorts
    NO external links. NO 'subscribe' / 'like' / 'follow' calls.",
  "tags": ["8-12 lowercase tags, no '#' prefix, no spaces in single tags. Mix:
            - 3 broad niche tags ('space', 'astronomy', 'cosmos', 'universe', 'science')
            - 5 specific tags about the actual subject ('blackhole', 'galaxy', 'planet')
            - 2 trending ('shorts', 'fyp', 'spacefacts', 'astronomy')"]
}
...
def _gunluk_tavan_kontrolu(tavan: int = 4) -> None:
    """Slot kararı ortak `slot_kapisi` modülünden gelir (tek kaynak).
    Workflow zaten ilk adımda aynı kapıyı çalıştırıyor; bu ikinci kapı,
    elle tetik / kod hatası gibi yollardan gelen fazla basımı da keser."""
    import sys as _sys
    from hedef_saatler import HEDEF_SAATLER
    from slot_kapisi import slot_karari
