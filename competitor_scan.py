...
RAKIP_KANALLAR = [
    ...
    {"name": "PBS Space Time", "handle": "@pbsspacetime"},
    {"name": "Sea Lab", "handle": "@AnswersWithJoe"},
    {"name": "SciShow Space", "handle": "@scishowspace"},
    {"name": "Astrum", "handle": "@AstrumSpace"},
    {"name": "Cool Worlds", "handle": "@CoolWorldsLab"},
]
...
def son_yayinlar(yt, uploads_pl: str, gun: int = 7) -> list[dict]:
    """uploads playlist'inden son N günlük yayınlar. 1 unit/sayfa (search 100 unit'in 1/100'ü).
    Tarih filtresi client tarafında yapılır."""
    after_dt = datetime.now(timezone.utc) - timedelta(days=gun)
    out = []
    try:
        # uploads zaten tarih-azalan sıralı, max 1 sayfa (50 video) yeter — son 7 günde 50'den fazla yayın olmaz
...
def main():
    # KOTA KORUMA: 24h cache (her run gereksiz, günde 1 yeter)
    if CIKTI.exists():
