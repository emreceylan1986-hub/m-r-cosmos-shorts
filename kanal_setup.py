...
def mevcut_sectionlar(yt):
    """Kanalın channelSections listesi (snippet)."""
    r = yt.channelSections().list(part="snippet,contentDetails", mine=True).execute()
    return r.get("items", [])


def section_var_mi(mevcut, type_, playlist_id=None, title=None):
    """Belirli section daha önce eklenmiş mi (duplikasyon önleme, case-insensitive)."""
    type_l = type_.lower()
    for sec in mevcut:
        sn = sec.get("snippet", {})
        ...
        if type_l == "singleplaylist":
            pls = cd.get("playlists", [])
            if playlist_id and playlist_id in pls:
                return True
        else:
            return True
    return False
