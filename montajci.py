...
# 9 Ağu: uzay/kozmik ton havuzu — canlı ölçümle seçildi (verylow tempo + ticari
# kullanıma açık ccnc=false parça VEREN terimler; çok sonuç veren başta).
KOZMIK_ARAMALAR = [
    "atmospheric drone",  # 6 parça
    "deep space",         # 5
    "sci fi ambient",     # 4
    "cinematic ambient",  # 3
    "dark ambient",       # 3
    "cosmic ambient", "futuristic ambient",
]
# 20 Tem telif dersi: bu parça (Adrev claim) bir Akasha videosunu dünya genelinde
# bloklatmıştı ve HÂLÂ arama sonuçlarında çıkıyor — asla seçilmesin.
...
_KEYWORD_YEDEK = ['deep space nebula', 'spiral galaxy stars', 'planet in orbit', 'cosmic dust clouds', 'milky way night sky', 'supernova explosion', 'star field zoom', 'aurora from space', 'sun solar flare', 'colorful nebula clouds']
    ...
    try:
        ...
        if not muzik_var:
            ...
            if not muzik_key:
                ...
                _havuz = _rnd.sample(KOZMIK_ARAMALAR, len(KOZMIK_ARAMALAR))
                muzik_var = False
